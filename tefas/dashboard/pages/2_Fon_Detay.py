"""Fon Detay: tek fonun büyümesi, drawdown'u, yuvarlanan metrikleri ve künyesi.
...
st.set_page_config(page_title="Fon Detay", page_icon="🔬", layout="wide")
...
code = st.selectbox("Fon seç", sorted(labels), index=sorted(labels).index(str(default_code)),
...
m2.metric("Yıllık getiri", f"%{num('Yillik_Getiri'):.1f}")
...
if prepped is None:
    st.info("Bu fon için yeterli fiyat geçmişi yok (<10 gözlem).")
    st.stop()
norm, dd = prepped
tt = t[mask]
...
fig.update_layout(title="Kümülatif Büyüme (baz=100)", height=360,
                  yaxis_title="Değer", margin=dict(t=40, b=10))
c_left.plotly_chart(fig, use_container_width=True)

fig_dd = go.Figure()
fig_dd.add_scatter(x=tt, y=-dd, mode="lines", fill="tozeroy", name="Drawdown",
                   line=dict(color="#c0392b", width=1))
fig_dd.update_layout(title="Sualtı / Drawdown (%)", height=360,
                     yaxis_title="Zirveye göre kayıp (%)", margin=dict(t=40, b=10))
c_right.plotly_chart(fig_dd, use_container_width=True)

# ── Yuvarlanan 63 günlük getiri & volatilite (paylaşılan prep_rolling) ───────
...
if rolled is not None:
    ...
    f1.update_layout(title="Yuvarlanan 63g Getiri (yıllık, %)", height=320, margin=dict(t=40, b=10))
    r1.plotly_chart(f1, use_container_width=True)
    f2 = go.Figure()
...
# ── Aylık getiriler (paylaşılan prep_monthly_returns) ────────────────────────
...
with st.expander("Tüm metrikler"):
    st.dataframe(row.to_frame("Değer"), use_container_width=True)
