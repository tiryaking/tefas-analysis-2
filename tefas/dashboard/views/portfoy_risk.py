...
with tab_model:
    if not model:
        ...
        # Üst satır: KPI'lar (sol) + tahsis pastası (sağ)
        kpi_col, pie_col = st.columns([1, 1.5])
        with kpi_col:
            st.metric("Beklenen yıllık getiri (ağırlıklı)", f"%{exp_ret:.1f}")
            st.metric("Fon sayısı", len(model))
            st.metric("Tema sayısı", int(port["Tema"].nunique()))
        with pie_col:
            st.plotly_chart(px.pie(port, names="Fon Kodu", values="Agirlik", hole=0.45)
                            .update_layout(height=300, margin=dict(t=10, b=10, l=10, r=10),
                                           legend=dict(orientation="h", y=-0.1)),
                            width="stretch")
        # Tablo TAM GENİŞLİK (yatay scroll yok): formatlı kolonlar + Fon Adı geniş
        ...
        if pivot is not None:
            ...
            codes = [str(p["Fon Kodu"]) for p in model]
